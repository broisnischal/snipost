...
/// A global shortcut: Carbon keycode + Carbon modifier mask, with conversions
/// for display, menu key-equivalents, and recording from NSEvents.
struct Hotkey: Equatable {
    ...
    /// Builds from a recorded key event; nil if the combo lacks a real
    /// modifier (⌘/⌥/⌃) and would swallow normal typing.
    init?(event: NSEvent) {
        ...
        self.carbonModifiers = Self.carbonModifiers(from: flags)
    }
    ...
    /// Single printable characters can double as NSMenuItem key equivalents.
